...
fn build_spidermonkey(build_dir: &Path) {
    ...
    if target.contains("wasi") && !target.contains("wasmer") {
        let wasi_sysroot =
...
fn cc_flags(bindgen: bool) -> Vec<&'static str> {
    ...
    if target.contains("windows") {
        ...
        // For WASIX, we don't want a PIC build since we don't need dlopen
        if !target.contains("wasmer") {
            flags.push("-fPIC");
        }
    ...
    if target.contains("wasi") {
        flags.extend(&["-m32", "-D_WASI_EMULATED_MMAN"]);
    }
